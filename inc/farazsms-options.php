...
class class_farazsms_base
{
    ...
    public function __construct()
    {
        ...
        $credentials_option = get_option('faraz_credentials');
        if ($credentials_option) {
            $faraz_uname = $credentials_option['faraz_uname'];
            $faraz_password = $credentials_option['faraz_password'];
            $admin_number = $credentials_option['faraz_admin_notify_number'];
            $faraz_apikey = $credentials_option['faraz_api_key'];
            $faraz_fromnum = $credentials_option['faraz_fromnum'];
            $faraz_fromnum_adver = $credentials_option['faraz_fromnum_adver'];
            $faraz_sendwm = $credentials_option['faraz_sendwm'];
            $faraz_welcomep = $credentials_option['faraz_welcomep'];
            $faraz_sendwm_with_pattern = $credentials_option['faraz_sendwm_with_pattern'];
            $faraz_welcome_message = $credentials_option['faraz_welcome_message'];
            if ($faraz_uname && $faraz_password && $faraz_fromnum) {
                self::$username = self::faraz_tr_num($faraz_uname);
                self::$password = self::faraz_tr_num($faraz_password);
                self::$admin_number = self::faraz_tr_num($admin_number);
                self::$fromNum = self::faraz_tr_num($faraz_fromnum);
                self::$fromNumAdver = self::faraz_tr_num($faraz_fromnum_adver);
                self::$sendwm = $faraz_sendwm === 'true';
                self::$welcomep = self::faraz_tr_num($faraz_welcomep);
                self::$sendwm_with_pattern = $faraz_sendwm_with_pattern === 'true';
                self::$welcome_message = self::faraz_tr_num($faraz_welcome_message);
            }
            self::$apiKey = $faraz_apikey;
        }
    ...
    public static function faraz_tr_num($str)
    {
    ...
    {
        $body = array(
            'username'       => self::faraz_tr_num($uname),
            'password'       => self::faraz_tr_num($pass),
        );
...
+982171333036', 'farazsms');
    ...
    {
        $faraz_admin_notify_pcode = self::faraz_tr_num(get_option('faraz_admin_notify_pcode'));
        if (empty($faraz_admin_notify_pcode) || empty(self::$admin_number)) {
            return;
        }
        $input_data = array();
        $patternMessage = self::get_registered_pattern_variables($faraz_admin_notify_pcode);
        if ($patternMessage === null) {
            return;
        }
        ...
        return self::farazsms_send_pattern($faraz_admin_notify_pcode, self::$admin_number, $input_data);
    }
    ...
    {
        $phone = self::faraz_tr_num($phone);
        $comment_phone_book = get_option('faraz_comment_phone_book', []);
    ...
    {
        $phone = self::faraz_tr_num($phone);
        $pattern = get_option('faraz_newsletter_newsletter_pcode');
    ...
    {
        $phone = self::faraz_tr_num($phone);
        $pattern = get_option('faraz_woo_checkout_otp_pattern');
        if (empty($phone) ||  empty($pattern) || empty($data)) {
            return FALSE;
        }
        $input_data = array();
    ...
    {
        $newsletter_welcome = get_option('faraz_newsletter_welcome', 'false');
        $newsletter_welcomep = get_option('faraz_newsletter_welcomep', '');
    ...
    {
        $faraz_woo_poll = get_option('faraz_woo_poll', 'false');
        $faraz_woo_poll_time = get_option('faraz_woo_poll_time', '');
        $faraz_woo_poll_message = get_option('faraz_woo_poll_message', '');
        if ($faraz_woo_poll == "false" || empty($faraz_woo_poll_time) || empty($faraz_woo_poll_message)) {
            return;
        }
        ...
        $message = str_replace(array(
            ...
            get_bloginfo("name"),
            $data["item_link"]
        ), $faraz_woo_poll_message);

        $date_to_send = date('Y-m-d H:i:s', strtotime($order_date->date("Y-m-d H:i:s") . ' + ' . $faraz_woo_poll_time . ' days'));
    ...
    {
        $tracking_code_pattern = get_option('faraz_woo_tracking_code_pattern', '');
