(function ($) {
  ...
  $(".tabs .tab .tab-switch").on("click", function () {
    ...
    var new_url = $.updateQueryStringParameter(
    ...
    submitHandler: function (form) {
      ...
      $.post(ajaxurl, data, function (response) {
        ...
        if (!response.success) {
          ...
          $("<span>اطلاعات با موفقیت ذخیره شد</span>").appendTo(
            response_message
          );
  ...
  var send_message_to_phonebooks_button = $(
    "#send_message_to_phonebooks_button"
  );
  ...
  $("#send_message_to_phonebooks_form").validate({
    ...
    rules: {
      ...
      fsms_phones_choice: {
        required: function (element) {
          return $("#fsms_send_formnum_choice").find(":selected").val() === "2"
            ? true
            : false;
        },
        minlength: 23,
    ...
    messages: {
      ...
      fsms_phones_choice:
        "برای ارسال پیامک تبلیغاتی وارد کردن حداقل 2 شماره موبایل اجباری می باشد",
    },
    submitHandler: function (form) {
      var data = {
  ...
  sync_buttons.click(function () {
    ...
    $.post(ajaxurl, data, function (response) {
      ...
      if (response.success) {
        $("<span>هماهنگ سازی با موفقیت انجام شد</span>").appendTo(
          fsms_sync_response
        );
  ...
  var comment_settings_response_message = $(
    "#fsms-comment-settings-response-message"
  );
  ...
  comment_settings_save_button.click(function () {
    ...
    $.post(ajaxurl, data, function (response) {
      ...
      if (response.success) {
        $("<span>اطلاعات با موفقیت ذخیره شد</span>").appendTo(
          comment_settings_response_message
        );
  ...
  $(
    ".fsms_form_element select,#comment_phone_book,#fsms_phonebooks_choice,#fsms_send_formnum_choice,#fsms_edd_phonebooks_choice,#fsms_newsletter_phonebooks,#fsms_admin_login_noti_roles"
  ).select2({
    theme: "classic",
    language: {
      noResults: function () {
  ...
  var newsletter_settings_save_button = $(
    "#fsms_newsletter_settings_save_button"
  );
  var newsletter_settings_response_message = $(
    "#fsms-newsletter-settings-response-message"
  );
  ...
  newsletter_settings_save_button.click(function () {
    var data = {
      ...
      newsletter_send_ver_code: $("#fsms_newsletter_send_ver_code").is(
        ":checked"
      ),
      ...
      newsletter_new_post_notification: $(
        "#fsms_newsletter_new_post_notification"
      ).is(":checked"),
      newsletter_post_notification_message: $(
        "#fsms_newsletter_post_notification_message"
      ).val(),
      newsletter_new_product_notification: $(
        "#fsms_newsletter_new_product_notification"
      ).is(":checked"),
      newsletter_product_notification_message: $(
        "#fsms_newsletter_product_notification_message"
      ).val(),
    };
    ...
    $.post(ajaxurl, data, function (response) {
      ...
      if (response.success) {
        $("<span>اطلاعات با موفقیت ذخیره شد</span>").appendTo(
          newsletter_settings_response_message
        );
  ...
  delete_subscriber.click(function () {
    var but = $(this);
    if (
      confirm("آیا مطمئن هستید که میخواهید این کاربر را از خبرنامه حذف کنید؟")
    ) {
      var data = {
      ...
      $.post(ajaxurl, data, function (response) {
        ...
        if (response.success) {
          $("<span>کاربر با موفقیت از خبرنامه حذف شد</span>").appendTo(
            newsletter_settings_response_message
          );
          newsletter_settings_response_message.addClass("fsms-success-message");
          newsletter_settings_response_message.show();
  ...
  var send_message_to_subscribers_response = $(
    "#send_message_to_subscribers_response"
  );
  ...
  var others_settings_response_message = $(
    "#fsms-others-settings-response-message"
  );
  ...
  $("#fsms_others_settings_save_form").validate({
    rules: {
      fsms_ihc_first_noti_sms_message: {
        required: function (element) {
          return $(
            "#fsms_ihc_send_first_noti_sms,#fsms_ihc_send_second_noti_sms,#fsms_ihc_send_third_noti_sms"
          ).is(":checked")
            ? true
            : false;
        },
      ...
      fsms_pmp_expire_noti_sms_message: {
        required: function (element) {
          return $("#fsms_pmp_send_expire_noti_sms").is(":checked")
            ? true
            : false;
        },
      },
      ...
      fsms_aff_admin_user_register_message: {
        required: function (element) {
          return $("#fsms_aff_admin_user_register").is(":checked")
            ? true
            : false;
        },
      },
      fsms_aff_admin_user_new_ref_message: {
        required: function (element) {
          return $("#fsms_aff_admin_user_new_ref").is(":checked")
            ? true
            : false;
        },
      },
      fsms_aff_admin_user_on_approval_message: {
        required: function (element) {
          return $("#fsms_aff_admin_user_on_approval").is(":checked")
            ? true
            : false;
        },
      },
    ...
    submitHandler: function (form) {
      var data = {
        ...
        ihc_send_first_noti_sms: $("#fsms_ihc_send_first_noti_sms").is(
          ":checked"
        ),
        ihc_send_second_noti_sms: $("#fsms_ihc_send_second_noti_sms").is(
          ":checked"
        ),
        ihc_send_third_noti_sms: $("#fsms_ihc_send_third_noti_sms").is(
          ":checked"
        ),
        ihc_first_noti_sms_message: $("#fsms_ihc_first_noti_sms_message").val(),
        pmp_send_expire_noti_sms: $("#fsms_pmp_send_expire_noti_sms").is(
          ":checked"
        ),
        pmp_expire_noti_sms_message: $(
          "#fsms_pmp_expire_noti_sms_message"
        ).val(),
        ...
        aff_user_on_approval_message: $(
          "#fsms_aff_user_on_approval_message"
        ).val(),
        aff_admin_user_register: $("#fsms_aff_admin_user_register").is(
          ":checked"
        ),
        aff_admin_user_register_message: $(
          "#fsms_aff_admin_user_register_message"
        ).val(),
        aff_admin_user_new_ref: $("#fsms_aff_admin_user_new_ref").is(
          ":checked"
        ),
        aff_admin_user_new_ref_message: $(
          "#fsms_aff_admin_user_new_ref_message"
        ).val(),
        aff_admin_user_on_approval: $("#fsms_aff_admin_user_on_approval").is(
          ":checked"
        ),
        aff_admin_user_on_approval_message: $(
          "#fsms_aff_admin_user_on_approval_message"
        ).val(),
      };
      ...
      $.post(ajaxurl, data, function (response) {
        ...
        if (!response.success) {
          $("<span>" + response.data + "</span>").appendTo(
            others_settings_response_message
          );
          others_settings_response_message
            .addClass("fsms-error-message")
            .show();
        } else {
          $("<span>اطلاعات با موفقیت ذخیره شد</span>").appendTo(
            others_settings_response_message
    ...
    submitHandler: function (form) {
      ...
      $.post(ajaxurl, data, function (response) {
        ...
        if (!response.success) {
          ...
          $("<span>پیام با موفقیت ارسال شد</span>").appendTo(
            feedback_response_message
          );
